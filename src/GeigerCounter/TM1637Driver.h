...
class TM1637
{
  public:
    ...
    void setInt(int32_t number, bool leadZeros = false);
    //! Show a float on the display.
    //!
    //! @param number The float to display
    //! @param leadZeros Should the remaining space be filled with zeros
    void setFloat(float number, bool leadZeros = false, uint8_t decimalPlaces = 2);
    //! Show a number on the display.
    //!
    //! @param number The number to display
    //! @param leadZeros Should the remaining space be filled with zeros
    void setNumber(int32_t number, bool leadZeros = false, uint8_t dot = 255);
  ...
  private:
    ...
    uint8_t _displayBuffer[5] = { 0x00, 0x00, 0x00, 0x00, 0x00 };
